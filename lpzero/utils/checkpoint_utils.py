...
def save_checkpoint(state, save_dir, ckpt_name, keep_num, is_best=False):
    ...
    # logging.info("save_path={}".format(save_dir))
    ...
    all_ckpt = np.array(
        [file for file in os.listdir(save_dir) if re.match(
            ckpt_head, file) is not None]
    )
    all_idx = np.int32([re.findall(r'\d+', ckpt)[0] for ckpt in all_ckpt])
    sorted_ckpt = all_ckpt[np.argsort(all_idx)[::-1]]
    remove_path = [os.path.join(save_dir, name)
                   for name in sorted_ckpt[keep_num:]]
...
def load_pretrain_state_dict(
    ...
    state_dict_path,
    add_module=False,
    load_lm_weights=False,
    is_finetune=False,
    is_spos=False,
    task='glue',
):
    raw_state_dict = torch.load(state_dict_path, map_location='cpu')
    # print(raw_state_dict)raw_state_dict
    if re.search('pretrained_ckpt', state_dict_path):
        is_finetune = False
    is_squad = False
    if re.search('squad', task):
        is_squad = True
    logging.info('is_squad={}'.format(is_squad))
    if is_finetune:
        raw_state_dict_backup = raw_state_dict
        try:
            raw_state_dict = raw_state_dict['state_dict']
        except:
            raw_state_dict = raw_state_dict_backup
        new_state_dict = {}
        for n, p in raw_state_dict.items():
            if (
                re.search(r'lm_head', n) is None
                and (re.search(r'classifier', n) is None or is_spos)
                and (re.search(r'cls_pooler', n) is None or not is_squad)
            ):
                n = re.sub(r'module\.', '', n)
        ...
        for n, p in raw_state_dict.items():
            ...
            if (
                model_name in models.bert_models + models.roberta_models
                and re.search(r'pooler|cls', n) is None
            ):
                if (
                    re.match(r'roberta', n) is not None
                    and re.search(r'token_type_embeddings', n) is not None
                ):
                    continue
            ...
            if model_name in models.xlnet_models and re.search(r'mask_emb', n) is None:
                n = re.sub(r'layer\.', '', n)
                n = re.sub(
                    r'transformer\.word_embedding', 'base_model.token_embeddings', n
                )
            ...
            # Bert LM weights
            if (
                model_name in models.bert_models
                and load_lm_weights
                and re.match(r'cls\.predictions', n) is not None
            ):
...
def load_multi_task_state_dict(
    model, state_dict_path, task_id, is_finetune=False, load_pred=True
):
    raw_state_dict = torch.load(state_dict_path, map_location='cpu')[
        'state_dict']
    new_state_dict = {}
    for n, p in raw_state_dict.items():
        if is_finetune:
            if load_pred:
                ...
                if (
                    re.search(r'classifiers', n) is None
                    and re.search(r'cls_pooler', n) is None
                ):
                    new_state_dict[n] = p
        else:
            res = re.search(r'classifier', n)
            if res is not None:
                n = n[: res.start()] + 'classifiers.' + \
                    str(task_id) + n[res.end():]
            new_state_dict[n] = p
...
def load_supernet_state_dict_6_540_to_6_360(
    model, state_dict_path, raw_dim=540, new_dim=360
):
    def _adj_param_dim(name, param):
        if re.search(r'embeddings', name) is not None:
            ...
            if (
                re.search(r'weight', name) is not None
                and re.search(r'layernorm', name) is None
            ):
            ...
            raise KeyError(
                'Do not support adjusting param dim of {}'.format(name))
        return param
...
def load_supernet_state_dict_6_540_to_12_360(
    model, state_dict_path, raw_dim=540, new_dim=360
):
    def _adj_param_dim(name, param):
        if re.search(r'embeddings', name) is not None:
    ...
    for n, p in raw_state_dict.items():
        ...
        if res is not None:
            ...
            new_n1 = n[: res.start()] + 'encoder.' + \
                str(new_layer_id1) + n[res.end():]
            new_n2 = n[: res.start()] + 'encoder.' + \
                str(new_layer_id2) + n[res.end():]
