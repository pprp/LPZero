...
def load_pretrain_state_dict(model_name, model, state_dict_path, add_module=False, load_lm_weights=False, is_finetune=False, is_spos=False, task='glue'):
    ...
    if is_finetune:
        ...
        for n, p in raw_state_dict.items():
            if re.search(r'lm_head', n) is None and (re.search(r'classifier', n) is None or is_spos) and (re.search(r'cls_pooler',n) is None or not is_squad):
                n = re.sub(r'module\.', '', n)
