...
InputFeatures = namedtuple(
    'InputFeatures', 'token_ids segment_ids position_ids attn_mask lm_label_ids is_next'
)
...
class PretrainDataset(Dataset):
    def __init__(
        ...
        train_ratio=0.9,
        val_ratio=0.1,
        concate_data_dir=None,
        read_only=True,
    ):
        data_epoch = int(epoch % num_data_epochs)

        if not isinstance(all_data_dir, list) or (
            isinstance(all_data_dir, list) and len(all_data_dir) == 1
        ):
            ...
            (
                ...
                lm_label_ids,
                is_nexts,
            ) = self.process_single_task_dataset(
                data_dir, data_epoch, tokenizer, split, local_rank, read_only
            )
            ...
            if split == 'train':
                ...
                assert 0 < self.num_samples <= int(
                    num_samples * (1 - train_ratio)) + 1
                self.seq_len = seq_len
                self.token_ids = token_ids[-self.num_samples:]
                self.segment_ids = segment_ids[-self.num_samples:]
                self.position_ids = position_ids[-self.num_samples:]
            ...
            for data_dir in all_data_dir:
                metrics_file = data_dir / \
                    'epoch_{}_metrics.json'.format(data_epoch)
                metrics = json.loads(metrics_file.read_text())
                cur_num_samples = metrics['num_training_examples']
                seq_len = metrics['max_seq_len']
                total_num_samples += cur_num_samples

            concate_save_dir = concate_data_dir / \
                ('data_epoch_' + str(data_epoch))
            if concate_save_dir.exists():
                if local_rank == 0:
                    logging.info(
                        "Loading {} concate pretrain dataset from '{}'".format(
                            split, concate_save_dir
                        )
                    )

                concate_token_ids = np.memmap(
                    filename=concate_save_dir / 'token_ids.memmap',
                    shape=(total_num_samples, seq_len),
                    mode='r',
                    dtype=np.int32,
                )
                concate_segment_ids = np.memmap(
                    filename=concate_save_dir / 'segment_ids.memmap',
                    shape=(total_num_samples, seq_len),
                    mode='r',
                    dtype=np.int32,
                )
                concate_position_ids = np.memmap(
                    filename=concate_save_dir / 'position_ids.memmap',
                    shape=(total_num_samples, seq_len),
                    mode='r',
                    dtype=np.int32,
                )
                concate_attn_mask = np.memmap(
                    filename=concate_save_dir / 'attn_mask.memmap',
                    shape=(total_num_samples, seq_len),
                    mode='r',
                    dtype=np.int32,
                )
                concate_lm_label_ids = np.memmap(
                    filename=concate_save_dir / 'lm_label_ids.memmap',
                    shape=(total_num_samples, seq_len),
                    mode='r',
                    dtype=np.int32,
                )
                concate_is_nexts = np.memmap(
                    filename=concate_save_dir / 'is_nexts.memmap',
                ...
                if read_only:
                    raise RuntimeError(
                        'Pretrain dataset is currently read only, if the dataset has not been created yet, '
                        "please run 'create_pretrain_feature.py' to create it before training."
                    )

                if local_rank == 0:
                    logging.info(
                        "Writing {} concate pretrain dataset to '{}'".format(
                            split, concate_save_dir
                        )
                    )
            ...
            if split == 'train':
                ...
                assert (
                    0
                    < self.num_samples
                    <= int(total_num_samples * (1 - train_ratio)) + 1
                )
                self.seq_len = seq_len
                self.token_ids = concate_token_ids[-self.num_samples:]
                self.segment_ids = concate_segment_ids[-self.num_samples:]
    ...
    @staticmethod
    def process_single_task_dataset(
        data_dir, data_epoch, tokenizer, split, local_rank, read_only
    ):
        ...
        if save_dir.exists():
            if local_rank == 0:
                logging.info(
                    "Loading {} pretrain dataset from '{}'".format(
                        split, save_dir)
                )

            token_ids = np.memmap(
                filename=save_dir / 'token_ids.memmap',
                shape=(num_samples, seq_len),
                mode='r',
                dtype=np.int32,
            )
            segment_ids = np.memmap(
                filename=save_dir / 'segment_ids.memmap',
                shape=(num_samples, seq_len),
                mode='r',
                dtype=np.int32,
            )
            position_ids = np.memmap(
                filename=save_dir / 'position_ids.memmap',
                shape=(num_samples, seq_len),
                mode='r',
                dtype=np.int32,
            )
            attn_mask = np.memmap(
                filename=save_dir / 'attn_mask.memmap',
                shape=(num_samples, seq_len),
                mode='r',
                dtype=np.int32,
            )
            lm_label_ids = np.memmap(
                filename=save_dir / 'lm_label_ids.memmap',
                shape=(num_samples, seq_len),
                mode='r',
                dtype=np.int32,
            )
            is_nexts = np.memmap(
                filename=save_dir / 'is_nexts.memmap',
                shape=(num_samples,),
                mode='r',
                dtype=np.bool,
            )
            ...
            if local_rank == 0:
                logging.info(
                    "Writing {} pretrain dataset to '{}'".format(
                        split, save_dir)
                )
    ...
    def __getitem__(self, item):
        return (
            torch.tensor(self.token_ids[item], dtype=torch.long),
...
class MultiTaskBatchSampler(BatchSampler):
    ...
    def _gen_batch_indices(self, shuffle):
        ...
        for dataset in self.all_datasets:
            batch_indices = [
                list(range(i, min(i + self.batch_size, len(dataset))))
    ...
    def _gen_task_indices(self):
        ...
        for batch_task_id in range(len(self.batch_indices)):
            all_indices += [batch_task_id] * \
                len(self.batch_indices[batch_task_id])
        if self.shuffle:
        ...
        for batch_task_id in flatten_task_indices:
            ...
            if self.distributed:
                while len(samples) % self.num_replicas != 0:
                    samples += samples[
                        : (self.num_replicas - len(samples) % self.num_replicas)
                    ]
...
def create_pretrain_dataset(
    ...
    loader = _create_dataset_loader(
        dataset, batch_size, use_gpu, distributed, split, num_workers
    )
...
def create_multi_task_dataset(
    ...
    if split == 'train':
        train_batch_size = (
            batch_size * dist.get_world_size() if distributed else batch_size
        )
        loader = _create_multi_task_dataset_loader(
            all_datasets, train_batch_size, use_gpu, distributed, num_workers
        )
...
def _create_dataset_loader(
    dataset, batch_size, use_gpu, distributed, split, num_workers=4
):
    if distributed:
        sampler = DistributedSampler(
            dataset, shuffle=True if split == 'train' else False
        )
        # logging.info("sampler.seed={}".format(sampler.seed))
    else:
        sampler = (
            RandomSampler(
                dataset) if split == 'train' else SequentialSampler(dataset)
        )
...
def _create_multi_task_dataset_loader(
    all_datasets, batch_size, use_gpu, distributed, num_workers=4
):
    batch_sampler = MultiTaskBatchSampler(
        all_datasets, batch_size, distributed)
