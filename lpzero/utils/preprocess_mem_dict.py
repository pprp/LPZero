...
def calculate_metrics(mem_dict):
    ...
    for key, data in mem_dict.items():
        ...
        print(f"MI Score: {mi_score}, Silhouette Avg: {s_avg}, CCI Score: {cci_score}")
        print("===")
...
def plot_correlations(data):
    ...
    pairplot.fig.suptitle('Pairwise Plots of MI, Silhouette, Spearman Correlation, and CCI', y=1.02)
...
def main():
    ...
    if mem_dict:
        record_mi, record_s, record_sp, record_cci = calculate_metrics(mem_dict)
        record_s = (record_s - np.mean(record_s)) / np.std(record_s)  # Normalizing record_s
        
        plot_distribution(record_mi, "Distribution of Mutual Information Scores", "mi.png")
        improved_plot_distribution(record_s, "Normalized Distribution of Silhouette Scores", "s.png")
        plot_distribution(record_sp, "Distribution of Spearman Correlation Scores", "sp.png")
        data = pd.DataFrame({
