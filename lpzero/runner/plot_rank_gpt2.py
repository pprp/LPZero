...
from lpzero.nas_utils.constraints.constraint_pipeline import TorchConstraintPipeline


def get_metrics(
    topk, sorted_ground_truth, sorted_target, ground_truth, target, common_configs=None
):
    ...
    topk_val_ppl_list_target = [
        target[i] for i in range(len(target)) if i in sorted_ground_truth_binned
    ]
...
def get_parser():
    ...
    parser.add_argument("--exp_name", type=str, help="name of maulet experiment", default='random_GPT2_lm1b')
...
def main(args):
    ...
    common_configs = np.intersect1d(
        list(logs_val_ppls.keys()), list(logs_n_params.keys())
    )
    ...
    print(
        "decoder param count range:",
        np.min(np.absolute(n_params_nonemb)),
        np.max(np.absolute(n_params_nonemb)),
    )
    print(
        "total param count range:",
        np.min(np.absolute(n_params_total)),
    ...
    for topk in topk_list:
        ...
        cr, src = get_metrics(
            ...
            sorted_target=sorted_nparams_nonemb,
            ground_truth=val_ppls,
            target=n_params_nonemb,
        )
        ...
        cr, src = get_metrics(
            topk,
            sorted_ground_truth=sorted_val_ppl,
            sorted_target=sorted_nparams_total,
            ground_truth=val_ppls,
            target=n_params_total,
        )
    ...
    plt.scatter(
        range(1, len(sorted_nparams_nonemb) + 1),
        np.asarray(ranks_ground_truth) + 1,
        s=8,
    )
    plt.plot(
        range(1, len(sorted_nparams_nonemb) + 1),
    ...
    c = [
        colors[unique_div_vals.index(np.asarray(div_vals)[sorted_val_ppl][i])]
        for i in range(len(n_params_total))
    ]
    for subcolor in np.unique(c):
        plt.scatter(
            -np.asarray(n_params_total)[sorted_val_ppl][np.asarray(c) == subcolor],
            np.asarray(val_ppls)[sorted_val_ppl][np.asarray(c) == subcolor],
            s=25,
            c=subcolor,
            label=r"$k$={}".format(unique_div_vals[colors.index(subcolor)]),
        )
    ...
    plt.legend()
    plt.savefig(
        os.path.join(args.output_dir, "pareto_params_total.png"), bbox_inches="tight"
    )
    ...
    plt.savefig(
        os.path.join(args.output_dir, "commonratio_spearman.png"), bbox_inches="tight"
    )
