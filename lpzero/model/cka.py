...
def cka_score(x1: Tensor, x2: Tensor, gram: bool = False) -> Tensor:
    ...
    cross_score = linear_hsic(x1, x2, False)
    self_score1 = linear_hsic(x1, x1, False)
    self_score2 = linear_hsic(x2, x2, False)
...
class CKA_Minibatch(Module):
    ...
    def update(self, x1: Tensor, x2: Tensor, gram: bool = False) -> None:
        ...
        if not gram:
            x1 = torch.matmul(x1, x1.transpose(0, 1))
            x2 = torch.matmul(x2, x2.transpose(0, 1))
        self.cross_hsic.append(linear_hsic(x1, x2, False))
        self.self_hsic1.append(linear_hsic(x1, x1, False))
        self.self_hsic2.append(linear_hsic(x2, x2, False))
