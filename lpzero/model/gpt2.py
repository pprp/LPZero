...
class Gpt2Embedding(nn.Module):
    def __init__(self, config, num_add_tokens=0):
        ...
        self.token_embeddings = nn.Embedding(
            config.vocab_size + num_add_tokens, config.hidden_size
        )
...
class Gpt2Attention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer(
            'bias', torch.tril(torch.ones(n_ctx, n_ctx)
    ...
    def forward(self, hidden_states, attn_mask):
        ...
        query = query.view(
            x.size(0), -1, self.num_attn_heads, self.attn_head_size
        ).permute(0, 2, 1, 3)
        key = key.view(x.size(0), -1, self.num_attn_heads, self.attn_head_size).permute(
            0, 2, 1, 3
        )
        value = value.view(
            x.size(0), -1, self.num_attn_heads, self.attn_head_size
        ...
        attn_score = torch.where(
            mask.bool(), attn_score, self.masked_bias.to(attn_score.dtype)
        )  # Mask the upper triangular part of the attention score
...
class Gpt2(nn.Module):
    def __init__(self, config, num_add_tokens=0, use_lm=False, return_hid=False):
        ...
        self.encoder = nn.ModuleList(
            [Gpt2TransformerBlock(config) for _ in range(config.num_layers)]
        )
        self.layernorm = nn.LayerNorm(config.hidden_size)

        if self.use_lm:
            self.lm_head = nn.Linear(
                config.hidden_size, config.vocab_size + num_add_tokens, bias=False
            )
