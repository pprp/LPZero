...
class ChunkedLSAttention(nn.Module):
    def __init__(
        ...
        self.r_r_bias = nn.Parameter(
            torch.FloatTensor(1, self.n_head, 1, self.d_head))
        self.r_w_bias = nn.Parameter(
            torch.FloatTensor(1, self.n_head, 1, 1, self.d_head)
        )
    ...
    def compute_scores(self, h_vecs):
        ...
        h_scores = self.dconv_1(h_vecs).view(
            bsz, n_chunks, self.chunk_size, self.n_head, self.chunk_rank
        )
    ...
    def compress_chunks(self, h_vecs, h_scores):
        ...
        h_vecs = h_vecs.view(
            -1, n_chunks, self.chunk_size, self.n_head, self.d_h
        ).permute(0, 3, 1, 2, 4)
        # bsz x n_heads x n_chunks x chunk_rank x d_h
        h_vecs = h_scores.matmul(h_vecs).view(
            bsz, self.n_head, n_chunks * self.chunk_rank, self.d_h
        )
        return h_vecs
    ...
    def put_tiles(self, x):
        ...
        if bp_len > window_len:
            x = x.view(bsz, n_heads, bp_len //
                       window_len, window_len, window_len)
            out_size = (bsz, n_heads, bp_len // window_len,
        ...
        stride = x.stride()
        out_stride = (stride[0], stride[1], stride[2],
                      stride[3] - 1, stride[4])
    ...
    def get_diagonals(self, attn):
        ...
        out_stride = (in_stride[0], in_stride[1],
                      in_stride[2], in_stride[3] + 1, 1)
        return (
            attn.as_strided(size=out_size, stride=out_stride, storage_offset=1)
            .contiguous()
            .view(bsz, n_heads, -1, self.window_len)
        )
    ...
    def _rel_shift_chunked(self, x, chunk_size, chunk_rank):
        ...
        x = (
            x.view(bsz, n_head, n_q_chunks, chunk_size, n_c_vecs)
            .transpose(2, 3)
            .contiguous()
        )
        ...
        out_strides = (
            ...
            p_stride[1],
            p_stride[2],
            p_stride[3] - chunk_rank,
            p_stride[4],
        )

        x = x.as_strided(
            size=out_shape, stride=out_strides, storage_offset=n_q_chunks * chunk_rank
        )
    ...
    def attn(
        ...
        if seq_len > self.window_len:
            query_tile = query.view(
                bsz, n_heads, seq_len // self.window_len, self.window_len, d_h
            )
            key_window = self.get_tiles(key_window, seq_len, transpose=True)
            val_window = self.get_tiles(
                val_window, seq_len, transpose=False
            )  # bsz x n_heads x n_queries//wlen x 2*wlen x d//n_heads
        else:
            query_tile = query.view(bsz, n_heads, 1, seq_len, d_h)
            key_window = key_window.view(bsz, -1, self.n_head, d_h).permute(0, 2, 3, 1)[
                :, :, None, :, :
            ]
            val_window = val_window.view(bsz, -1, self.n_head, d_h).permute(0, 2, 1, 3)[
                :, :, None, :, :
            ]
        ...
        pos_trans = (
            self.r_net(pos_embed_window)
            .view(1, self.window_len, self.n_head, self.d_head)
            .permute(0, 2, 3, 1)
        )
        ...
        pos_chunks = (
            self.r_net_chunk(pos_embed_chunks)
            .view(1, n_chunks, self.n_head, self.d_head)
            .permute(0, 2, 3, 1)
        )

        attn_pos = torch.matmul(query, pos_chunks)  # B x H x M x L_pos
        attn_pos = self._rel_shift_chunked(
            attn_pos, self.chunk_size, self.chunk_rank)
        ...
        if chunk_attn_mask is not None:
            ...
            attn_compress = attn_compress.masked_fill(
                chunk_attn_mask, float('-inf'))
        ...
        out = torch.matmul(attn_compress, value_compressed) + self.compute_pv(
            attn_window, val_window
        )
    ...
    def forward(self, h, h_cache, key_pe, pos_embed_window, chunked_attn_mask=None):
        if self.grad_chk:
            ...
            out = self.forward_(h, h_cache, key_pe,
                                pos_embed_window, chunked_attn_mask)
        return out
    ...
    def forward_(self, h, h_cache, key_pe, pos_embed_window, chunked_attn_mask=None):
        ...
        h_cache_merge = (
            h_cache_compressed.view(bsz, self.n_head, -1, self.d_h)
            .transpose(1, 2)
            .contiguous()
        ...
        val_bp_compressed = self.dual_ln_dproj(
            val_bp_compressed.transpose(
                1, 2).contiguous().view(bsz, -1, self.d_model)
        )
        key_bp_compressed = self.dual_ln_dproj(
            key_bp_compressed.transpose(
                1, 2).contiguous().view(bsz, -1, self.d_model)
        )
