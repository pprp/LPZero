...
class XlnetAttention(nn.Module):
    def __init__(self, config):
        ...
        self.q = nn.Parameter(
            torch.FloatTensor(
                config.hidden_size, self.num_attn_heads, self.attn_head_size
            )
        )
        self.k = nn.Parameter(
            torch.FloatTensor(
        ...
        self.r_w_bias = nn.Parameter(
            torch.FloatTensor(self.num_attn_heads, self.attn_head_size)
        )
        ...
        self.seg_mat = nn.Parameter(
            torch.FloatTensor(2, self.num_attn_heads, self.attn_head_size)
        )
    ...
    def forward(self, hidden_states, position_embed, segment_embed, attn_mask):
        ...
        attn_out = self.hidden_dropout(
            torch.einsum('ibnd,hnd->ibh', attn_out, self.o))
...
class XlnetBaseModel(nn.Module):
    ...
    def forward(self, token_ids, segment_ids, attn_mask):
        ...
        non_target_mask = ((attn_mask + non_target_mask[:, :, None, None]) > 0).to(
            attn_mask
        )

        token_embed = self.dropout(self.token_embeddings(token_ids))
        segment_embed = (segment_ids[:, None] != segment_ids[None, :]).long()
        segment_embed = F.one_hot(segment_embed, num_classes=2).to(
    ...
    def _get_position_embedding(self, seq_len, bsz):
        ...
        pos_seq = torch.arange(seq_len, -seq_len, -1.0)
...
class XlnetClsPooler(nn.Module):
    ...
    def forward(
        self, hidden_states, cls_index, start_positions=None, start_states=None
    ):
        ...
        if start_positions is not None:
            ...
            start_states = hidden_states.gather(1, start_positions).squeeze(
                1
        ...
        cls_token_state = hidden_states.gather(1, cls_index).squeeze(
            1
        )  # (batch_size, hidden_size)
...
class StartLogitPooler(nn.Module):
    ...
    def forward(self, hidden_states, p_mask):
        ...
        big_num = 65500 if next(
            self.parameters()).dtype == torch.float16 else 1e30
...
class XlnetSingle(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = nn.ModuleList(
            [XlnetTransformerBlock(config) for _ in range(config.num_layers)]
        )
        self._init_weights()

    def forward(self, token_ids, segment_ids, position_ids, attn_mask):
        all_hidden_states = []
        token_embed, segment_embed, position_embed, attn_mask = self.base_model(
    ...
    def forward(
        ...
        if self.task in datasets.glue_tasks:
            ...
            if start_positions is None:
                ...
                cls_logits = self.cls_pooler(
                    output, cls_index, start_positions)
                ...
                )  # (batch_size, start_n_top)

                start_top_index_exp = start_top_index.unsqueeze(-1).expand(
                    -1, -1, hidden_size
                )  # (batch_size, start_n_top, hidden_size)
                start_states = torch.gather(output, 1, start_top_index_exp)
                start_states = start_states.unsqueeze(1).expand(
                    -1, seq_len, -1, -1
                )  # (batch_size, seq_len, start_n_top, hidden_size)
                output_exp = output.unsqueeze(2).expand_as(
                    start_states
                )  # (batch_size, seq_len, start_n_top, hidden_size)
                p_mask = p_mask.unsqueeze(-1)
                end_logits = self.end_pooler(
                    output_exp, p_mask, start_states=start_states
                )
                end_log_probs = F.softmax(end_logits, dim=-1)
                end_top_log_probs, end_top_index = torch.topk(
                    end_log_probs, self.end_n_top, dim=-1
                )  # (batch_size, end_n_top, start_n_top)
                ...
                start_states = torch.einsum(
                    'blh,bl->bh', output, start_log_probs
                )  # Get the representation of START as weighted sum of hidden states
                cls_logits = self.cls_pooler(
                    output, cls_index, start_states=start_states
                )
                return (
                    start_top_log_probs,
                    start_top_index,
