...
def create_glue_examples(task, glue_dir, split):
    ...
    if task == 'mrpc':
        ...
        text_a_id, text_b_id, label_id = [
            3, 4, 0] if split != 'test' else [3, 4, None]
    elif task == 'mnli':
        data_path = os.path.join(
            glue_dir, 'MNLI', file_name if split == 'train' else split + '_matched.tsv'
        )
        text_a_id, text_b_id, label_id = [
            8, 9, -1] if split != 'test' else [8, 9, None]
    elif task == 'mnli-mm':
        data_path = os.path.join(
            glue_dir,
            'MNLI',
            file_name if split == 'train' else split + '_mismatched.tsv',
        )
        text_a_id, text_b_id, label_id = [
            8, 9, -1] if split != 'test' else [8, 9, None]
    elif task == 'ax':
        data_path = os.path.join(glue_dir, 'diagnostic', 'diagnostic.tsv')
        text_a_id, text_b_id, label_id = [1, 2, None]
    elif task == 'cola':
        data_path = os.path.join(glue_dir, 'CoLA', file_name)
        text_a_id, text_b_id, label_id = (
            [3, None, 1] if split != 'test' else [1, None, None]
        )
    elif task == 'sst-2':
        data_path = os.path.join(glue_dir, 'SST-2', file_name)
        text_a_id, text_b_id, label_id = (
            [0, None, 1] if split != 'test' else [1, None, None]
        )
    elif task == 'sts-b':
        data_path = os.path.join(glue_dir, 'STS-B', file_name)
        text_a_id, text_b_id, label_id = [
            7, 8, -1] if split != 'test' else [7, 8, None]
    elif task == 'qqp':
        data_path = os.path.join(glue_dir, 'QQP', file_name)
        text_a_id, text_b_id, label_id = [
            3, 4, 5] if split != 'test' else [1, 2, None]
    elif task == 'qnli':
        data_path = os.path.join(glue_dir, 'QNLI', file_name)
        text_a_id, text_b_id, label_id = [
            1, 2, -1] if split != 'test' else [1, 2, None]
    elif task == 'rte':
    ...
    for i, line in enumerate(data):
        ...
        task_id = (
            datasets.glue_train_tasks_to_ids[task]
            if task in datasets.glue_train_tasks
            else None
        )
...
def create_glue_dataset(
    model_name, task, glue_dir, tokenizer, max_seq_len, split, local_rank, cache_dir=''
):
    ...
    cache_file = os.path.join(
        cache_dir, 'glue', '_'.join([model, task, split, str(max_seq_len)])
    )
    if tokenizer.lowercase:
        cache_file = os.path.join(
            cache_dir,
            'glue',
            '_'.join([model, task, split, str(max_seq_len), 'lowercase']),
        )
    ...
    if os.path.exists(cache_file):
        ...
        encoded_inputs = [
            ...
            for text_a, text_b in tqdm(
                zip(texts_a, texts_b), total=len(texts_a), disable=local_rank != 0
            )
        ]
    ...
    if split == 'test':
        ids = torch.tensor(
            [example.id for example in examples], dtype=torch.long)
        dataset = TensorDataset(token_ids, segment_ids,
                                position_ids, attn_mask, ids)
    else:
        labels = torch.tensor(
            [example.label for example in examples],
            dtype=torch.float if task == 'sts-b' else torch.long,
        )
        dataset = TensorDataset(token_ids, segment_ids,
                                position_ids, attn_mask, labels)
    ...
    for task in all_tasks:
        if split == 'dev':
            ...
            if split == 'train':
                ...
                assert 0 < num_val <= int(
                    len(cur_examples) * (1 - train_ratio)) + 1
...
def create_ensemble_glue_dataset(
    model_name,
    all_tasks,
    glue_dir,
    tokenizer,
    ...
    if split == 'train':
        file_name = '_'.join(
            [model, split, str(max_seq_len), str(train_ratio)])
    elif split == 'val':
    ...
    if os.path.exists(cache_file):
        ...
        if local_rank == 0:
            logging.info('Creating {} examples'.format(split))
        examples = create_ensemble_glue_examples(
            glue_dir, all_tasks, train_ratio, val_ratio, split
        )
    ...
    token_ids = [
        torch.tensor(
            [inp.token_ids for inp in cur_encoded_inputs], dtype=torch.long)
        for cur_encoded_inputs in encoded_inputs
    ]
    segment_ids = [
        torch.tensor(
            [inp.segment_ids for inp in cur_encoded_inputs], dtype=torch.long)
        for cur_encoded_inputs in encoded_inputs
    ]
    position_ids = [
        torch.tensor(
            [inp.position_ids for inp in cur_encoded_inputs], dtype=torch.long)
        for cur_encoded_inputs in encoded_inputs
    ]
    attn_mask = [
        torch.tensor(
            [inp.attn_mask for inp in cur_encoded_inputs], dtype=torch.long)
        for cur_encoded_inputs in encoded_inputs
    ]
    task_ids = [
        torch.tensor(
            [example.task_id for example in cur_examples], dtype=torch.long)
        for cur_examples in examples
    ]
    labels = [
        torch.tensor([example.label for example in cur_examples],
                     dtype=torch.float)
        for cur_examples in examples
    ]
    ...
    all_datasets = [
        ...
        for cur_task_ids, cur_token_ids, cur_segment_ids, cur_position_ids, cur_attn_mask, cur_labels in zip(
...
def create_split_glue_dataset(
    ...
    if split == 'train':
        file_name = '_'.join(
            [model, task, split, str(max_seq_len), str(train_ratio)])
    elif split == 'val':
        file_name = '_'.join(
            [model, task, split, str(max_seq_len), str(val_ratio)])
    else:
        raise KeyError("split '{}' is not valid".format(split))
    ...
    if os.path.exists(cache_file):
        ...
        if local_rank == 0:
            logging.info('Creating {} examples'.format(split))
        examples = create_ensemble_glue_examples(
            glue_dir, task, train_ratio, val_ratio, split
        )[0]
        texts_a = [example.text_a for example in examples]
        texts_b = [example.text_b for example in examples]
