    ...
    if task == 'swag':
        ...
        for line in data[1:]:
            ...
            examples.append(MultiChoiceExample(
                question, contexts, endings, label, id))
        ...
        for cur_data in all_text:
            ...
            for i in range(len(cur_data['answers'])):
                ...
                examples.append(MultiChoiceExample(
                    question, contexts, endings, label))
    else:
...
def create_multi_choice_dataset(
    ...
    cache_file = os.path.join(
        cache_dir,
        'multi_choice',
        '_'.join([model, task, split, str(max_seq_len), str(max_query_len)]),
    )
    if tokenizer.lowercase:
        cache_file = os.path.join(
            ...
            'multi_choice',
            '_'.join(
                [model, task, split, str(max_seq_len), str(
                    max_query_len), 'lowercase']
            ),
        )
    ...
    token_ids = torch.tensor(
        [
            [choice_inp.token_ids for choice_inp in encoded_input]
    ...
    segment_ids = torch.tensor(
        [
            [choice_inp.segment_ids for choice_inp in encoded_input]
            for encoded_input in encoded_inputs
        ],
        dtype=torch.long,
    )
    position_ids = torch.tensor(
        [
            [choice_inp.position_ids for choice_inp in encoded_input]
            for encoded_input in encoded_inputs
        ],
        dtype=torch.long,
    )
    attn_mask = torch.tensor(
        [
            [choice_inp.attn_mask for choice_inp in encoded_input]
            for encoded_input in encoded_inputs
        ],
        dtype=torch.long,
    )
