...
def attention_layer(
    ...
    if is_dft:
        ...
        # `input_layer` = [B, N, F, H]
        input_layer = transpose_for_scores(
            input_layer, batch_size, num_attention_heads, from_seq_length, size_per_head
        )
        # `fourier_layer` = [B, N, F, H]
        fourier_score = tf.math.real(
            tf.keras.layers.Lambda(lambda v: tf.signal.fft(tf.cast(v, tf.complex64)))(
                input_layer
            )
        )
        ...
        # `fourier_layer` = [B, N, F, H]
        cosine_score = tf.math.real(
            tf.keras.layers.Lambda(lambda v: tf.signal.dct(v))(input_layer)
        )
        ...
        # [B,T, N*H]
        key_conv_attn_layer = reshape_for_conv(
            to_tensor_2d, batch_size, num_attention_heads, to_seq_length, size_per_head
        )
        key_conv_attn_layer = tf.layers.separable_conv1d(
            key_conv_attn_layer,
            num_attention_heads * size_per_head,
            conv_kernel_size,
            padding='same',
        ...
        conv_out_layer = tf.reshape(
            ...
            [
                batch_size * to_seq_length * num_attention_heads,
                size_per_head,
                conv_kernel_size,
            ],
        )
            ...
            with tf.variable_scope('layer_%d' % layer_idx):
                with tf.variable_scope('attention'):
                    if hidden_size % encoder_layer['num_operation_heads'] != 0:
                        raise ValueError(
                            'The hidden size (%d) is not a multiple of the number of attention '
                            'heads (%d)'
                            % (hidden_size, encoder_layer['num_operation_heads'])
                        )
                ...
                with tf.variable_scope('intermediate'):
                    ...
                    for interediate_idx in range(encoder_layer['num_feed_forward'] - 1):
                        with tf.variable_scope(
                            'intermediate_layer_%d' % (interediate_idx + 1)
                        ):
