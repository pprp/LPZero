...
class PretrainingModel(object):
    ...
    def __init__(
        ...
        if config.uniform_generator:
            ...
            if config.two_tower_generator:
                ...
                cloze_output = self._get_cloze_outputs(
                    unmasked_inputs, generator)
    ...
    def _get_discriminator_output(
        ...
        with tf.variable_scope('discriminator_predictions'):
            ...
            DiscOutput = collections.namedtuple(
                'DiscOutput', ['loss', 'per_example_loss',
                               'probs', 'preds', 'labels']
            )
            return DiscOutput(
                loss=loss,
    ...
    def _get_fake_data(self, inputs, mlm_logits):
        ...
        if self._config.electric_objective:
            ...
            labels = masked * (
                1 - tf.cast(tf.equal(updated_input_ids,
                            inputs.input_ids), tf.int32)
            )
    ...
    def _get_cloze_outputs(self, inputs: pretrain_data.Inputs, model):
        ...
        weights = tf.cast(
            pretrain_helpers.get_candidates_mask(
...
class TwoTowerClozeTransformer(object):
    ...
    def __init__(
        ...
        self._sequence_output = tf.concat(
            [roll(ltr_reprs, -1), roll(rtl_reprs, 1)], -1)
